...
import { getDailyOfficeExpenseAction } from '@/features/daily-office-expenses'

interface DailyOfficeExpenseDetailsPageProps {
    params: Promise<{
        tenant_slug: string
        id: string
    }> | {
...
export default async function DailyOfficeExpenseDetailsPage({ params }: DailyOfficeExpenseDetailsPageProps) {
    ...
    const res = await getDailyOfficeExpenseAction(tenantSlug, expenseId)
        ...
        <div className="mx-auto space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">{expense.paid_to}</h1>
                        <p className="mt-1 text-sm text-slate-600">{expense.category}</p>
                    </div>

                    <div className="flex items-center gap-2">
                        <Link
                            href={`/${tenantSlug}/daily-office-expenses`}
                            className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100"
                        >
                            Back
                        </Link>
                        <Link
                            href={`/${tenantSlug}/daily-office-expenses/${expense.id}/edit`}
            ...
            <section className="grid gap-6 md:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 className="text-lg font-semibold text-slate-900">Expense Info</h2>
                    <div className="mt-4 space-y-3 text-sm">
                        ...
                        <p><span className="font-medium text-slate-700">Paid To:</span> <span className="text-slate-600">{expense.paid_to}</span></p>
