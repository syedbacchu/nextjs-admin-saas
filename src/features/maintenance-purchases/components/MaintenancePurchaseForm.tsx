...
import {
    createMaintenancePurchaseClient,
    updateMaintenancePurchaseClient,
    MaintenancePurchase
...
interface MaintenancePurchaseFormProps {
    tenantSlug: string
    offices: Office[]
    suppliers: Supplier[]
    vehicles: Vehicle[]
    ...
    initialData?: Partial<MaintenancePurchase>
    maintenancePurchaseId?: number | string
    submitLabel?: string
}
...
export default function MaintenancePurchaseForm({
    tenantSlug,
    offices,
    suppliers,
    vehicles,
    ...
    initialFiles,
    initialData,
    maintenancePurchaseId,
    submitLabel = 'Save Maintenance Purchase',
}: MaintenancePurchaseFormProps) {
    ...
    const [vehicleId, setVehicleId] = useState(toStringValue(initialData?.vehicle_id ?? initialData?.vehicle?.id))
    ...
    const [serviceDate, setServiceDate] = useState(toDateInputValue(initialData?.service_date))
    const [nextServiceDate, setNextServiceDate] = useState(toDateInputValue(initialData?.next_service_date))
    const [documentRenewDate, setDocumentRenewDate] = useState(toDateInputValue(initialData?.document_renew_date))
    const [documentNextExpireDate, setDocumentNextExpireDate] = useState(toDateInputValue(initialData?.document_next_expire_date))
    ...
    const isDocumentCategory = category === 'documents'
    ...
    function handleCategoryChange(nextCategory: string) {
        ...
        if (nextCategory === 'documents') {
            setServiceDate('')
            setNextServiceDate('')
            return
        }

        setDocumentRenewDate('')
        setDocumentNextExpireDate('')
    }
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        if (isDocumentCategory) {
            if (!documentRenewDate.trim()) {
                toast.error('Document renew date is required for documents category')
                return
            }
            if (!documentNextExpireDate.trim()) {
                toast.error('Document next expire date is required for documents category')
                return
            }
        }
        ...
        try {
            ...
            formData.append('service_date', serviceDate.trim())
            formData.append('next_service_date', nextServiceDate.trim())
            formData.append('document_renew_date', documentRenewDate.trim())
            formData.append('document_next_expire_date', documentNextExpireDate.trim())
            ...
            items.forEach((item, index) => {
                ...
                formData.append(`items[${index}][total]`, (item.total.trim() || calculateTotal(item.quantity, item.unit_price)).trim())
            })
            ...
            const res = maintenancePurchaseId
                ? await updateMaintenancePurchaseClient(tenantSlug, maintenancePurchaseId, formData)
                : await createMaintenancePurchaseClient(tenantSlug, formData)

            if (res.success) {
                toast.success(res.message || 'Maintenance purchase saved successfully')
                router.push(`/${tenantSlug}/maintenance-purchases`)
                router.refresh()
            } else {
                toast.error(res.message || 'Failed to save maintenance purchase')
            }
        } catch (error: unknown) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div>
                ...
                <p className="mt-1 text-sm text-slate-600">Fill in maintenance purchase details and save.</p>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                />
                {!isDocumentCategory && (
                    <DateInput label="Service Date" name="service_date" value={serviceDate} onChange={setServiceDate} />
                )}
                {!isDocumentCategory && (
                    <DateInput label="Next Service Date" name="next_service_date" value={nextServiceDate} onChange={setNextServiceDate} />
                )}
                {isDocumentCategory && (
                    <DateInput label="Document Renew Date" name="document_renew_date" value={documentRenewDate} onChange={setDocumentRenewDate} required />
                )}
                {isDocumentCategory && (
                    <DateInput
                        label="Document Next Expire Date"
                        name="document_next_expire_date"
                        value={documentNextExpireDate}
                        onChange={setDocumentNextExpireDate}
                        required
                    />
                )}
                <TextInput
            ...
            <div className="flex items-center gap-3">
                ...
                <button
                    type="button"
                    onClick={() => router.push(`/${tenantSlug}/maintenance-purchases`)}
