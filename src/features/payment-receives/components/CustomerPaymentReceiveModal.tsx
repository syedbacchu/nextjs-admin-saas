...
import { createCustomerPaymentReceiveAction } from '@/features/payment-receives/actions/trip-payment-receive.actions'
import {FileSystemItem} from "@/features/files";

interface CustomerPaymentReceiveModalProps {
  isOpen: boolean
  onClose: () => void
  customerId: number
  customerName: string
...
export default function CustomerPaymentReceiveModal({
  isOpen,
  onClose,
  customerId,
  customerName,
  ...
  initialFiles,
  onSuccess,
}: CustomerPaymentReceiveModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createCustomerPaymentReceiveAction(tenantSlug, {
      ...
      if (result.success) {
        toast.success(result.message || 'Payment received successfully')
        onSuccess?.()
        onClose()
        setAmount('')
        setNote('')
        ...
        toast.error(result.message || 'Failed to receive payment')
      }
    } catch (error) {
      console.error('Error creating payment receive:', error)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-purple-900/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="mb-4">
          <h3 className="text-xl font-bold text-slate-900">Receive Payment from Customer</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 rounded-lg bg-purple-800 px-4 py-2 font-medium text-white hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
