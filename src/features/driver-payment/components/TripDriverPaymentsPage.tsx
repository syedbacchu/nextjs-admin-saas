...
import {DriverPaymentSummary, getDriverPaymentSummariesAction} from "@/features/driver-payment";
import DriverPaymentModal from "@/features/driver-payment/components/DriverPaymentModal";
import DriverPaymentHistoryModal from "@/features/driver-payment/components/DriverPaymentHistoryModal";

type DriverPaymentRow = DriverPaymentSummary & { id: number; _serial: number }
...
export default function TripDriverPaymentsPage() {
    ...
    const [selectedDriver, setSelectedDriver] = useState<{ id: number; name: string; dueAmount: number } | null>(null)

    const fetchDropdownData = useCallback(async () => {
        try {
            const [driversRes, officesRes, filesRes] = await Promise.all([
    ...
    const { refreshKey, setRefreshKey, optimizedFetchData, tempFilters, buildApiFilters, handleFilterChange, applyFilters, resetFilters } = useOptimizedListPage({
        ...
        fetchListData: useCallback(async (page: number, search: string, filters?: Record<string, string>) => {
            ...
            try {
                ...
                const result = await getDriverPaymentSummariesAction(tenantSlug, params.toString())
                if (!result.success || !result.data) return EMPTY_DATA

                if (result.data.summary) {
                    const summaryData = result.data.summary
                    setSummary({
                        total_drivers: summaryData.total_drivers,
                        total_trips: summaryData.total_trips,
                        total_commission: parseFloat(summaryData.total_commission) || 0,
        ...
        fetchDropdownData,
        initialFilters: { driver_id: '', office_id: '', from_date: '', to_date: '' },
        enableInitialDataLoad: true,
    })

    const columns: ColumnDef<DriverPaymentRow>[] = [
        { header: '#', cell: (item) => <span className="text-slate-500">{item._serial}</span>, className: 'w-12' },
        { header: 'Driver', cell: (item) => <span className="font-medium">{item.driver?.name || 'N/A'}</span> },
        { header: 'Branch', cell: (item) => <span>{item.office?.branch_name || 'N/A'}</span> },
        { header: 'Trip Count', cell: (item) => <span>{item.trip_count}</span> },
        { header: 'Total Commission', cell: (item) => <span className="font-medium">{formatAmount(item.total_commission)}</span> },
        ...
        { header: 'Status', cell: (item) => getPaymentStatusBadge(item.payment_status) },
        {
            ...
            cell: (item) => (
                <div className="flex gap-2 justify-end">
                    ...
                    <button onClick={() => handleViewTrips(item.driver_id)} className="rounded-lg border border-blue-300 bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100" title="View driver trips">Trips</button>
                </div>
            ),
        },
    ]

    function handlePayNow(driver: DriverPaymentSummary) {
        setSelectedDriver({ id: driver.driver_id, name: driver.driver?.name || 'Unknown', dueAmount: driver.due_amount })
        setPaymentModalOpen(true)
    }

    function handleViewHistory(driver: DriverPaymentSummary) {
        setSelectedDriver({ id: driver.driver_id, name: driver.driver?.name || 'Unknown', dueAmount: driver.due_amount })
    ...
    function handleViewTrips(driverId: number) {
        router.push(`/${tenantSlug}/trips?driver_id=${driverId}`)
    }

    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between"><h1 className="text-2xl font-bold text-slate-900">Driver Payments (Trip-wise)</h1></div>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-4"><div className="mb-2 text-sm font-medium text-blue-700">Total Drivers</div><div className="text-2xl font-bold text-blue-900">{summary.total_drivers}</div></div>
                ...
                <div className="rounded-lg border border-orange-200 bg-orange-50 p-4"><div className="mb-2 text-sm font-medium text-orange-700">Total Commission</div><div className="text-2xl font-bold text-orange-900">{formatAmount(summary.total_commission)}</div></div>
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4"><div className="mb-2 text-sm font-medium text-emerald-700">Paid Drivers</div><div className="text-2xl font-bold text-emerald-900">{summary.paid_count}</div></div>
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-4"><div className="mb-2 text-sm font-medium text-amber-700">Partial Drivers</div><div className="text-2xl font-bold text-amber-900">{summary.partial_count}</div></div>
                <div className="rounded-lg border border-rose-200 bg-rose-50 p-4"><div className="mb-2 text-sm font-medium text-rose-700">Unpaid Drivers</div><div className="text-2xl font-bold text-rose-900">{summary.unpaid_count}</div></div>
            </div>
            <div className="rounded-lg border border-slate-200 bg-white p-4">
                <div className="mb-4"><h3 className="text-sm font-semibold text-slate-900">Filters</h3></div>
                <div className="grid gap-4 md:grid-cols-4">
                    <label className="space-y-1"><span className="text-xs font-medium uppercase tracking-wide text-slate-500">Driver</span><select value={tempFilters.driver_id} onChange={(e) => handleFilterChange('driver_id', e.target.value)} className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200"><option value="">All Drivers</option>{drivers.map((driver) => <option key={driver.id} value={String(driver.id)}>{driver.name}</option>)}</select></label>
            ...
            <DynamicTable key={refreshKey} title="Driver Payment Summary" fetchData={optimizedFetchData} columns={columns} initialFilters={buildApiFilters} />
            {selectedDriver && <>
                <DriverPaymentModal isOpen={paymentModalOpen} onClose={() => setPaymentModalOpen(false)} driverId={selectedDriver.id} driverName={selectedDriver.name} dueAmount={selectedDriver.dueAmount} tenantSlug={tenantSlug} initialFiles={initialFiles} onSuccess={handlePaymentSuccess} />
                <DriverPaymentHistoryModal isOpen={historyModalOpen} onClose={() => setHistoryModalOpen(false)} tenantSlug={tenantSlug} driverId={selectedDriver.id} driverName={selectedDriver.name} onSuccess={handlePaymentSuccess} />
            </>}
        </div>
    )
}
