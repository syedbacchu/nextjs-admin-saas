...
import {
  deleteDriverPaymentHistoryClient,
  DriverPayment,
  getDriverPaymentHistoryAction
...
export default function DriverPaymentHistoryModal({
  isOpen,
  onClose,
  tenantSlug,
  driverId,
  ...
  const fetchPaymentHistory = useCallback(async () => {
    setLoading(true)
    try {
      const result = await getDriverPaymentHistoryAction(tenantSlug, driverId)
  ...
  async function handleDelete(paymentId: number) {
    ...
    try {
      const result = await deleteDriverPaymentHistoryClient(tenantSlug, driverId, paymentId)
          ...
          <div className="space-y-3">
            {payments.map((payment) => (
              ...
              >
                ...
                {payment.bill_ref && (
                  <div>
                    <span className="text-slate-600">Bill Reference:</span>
                    <p className="mt-1 text-sm text-slate-800 font-medium">{payment.bill_ref}</p>
                  </div>
                )}
                {payment.note && (
                  <div>
